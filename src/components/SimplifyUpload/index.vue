<template>
  <el-upload
    ...
    :list-type="drag ? 'picture-card' : 'text'"
    :accept="accept"
    :disabled="drag && limit <= fileList.length"
...
const uploadFileRequest = (options: UploadRequestOptions) => {
  return uploadFile({ file: options.file, type: props.dir, isAppendTime: props.isAppendTime })
}
...
const emitChange = () => {
  if (props.limit > 1) {
    ...
    const name = fileList.value.map((item) => item.name)
    emits('update:modelValue', map)
    emits('getName', name)
  } else {
    emits('update:modelValue', fileList.value[0]?.url || '')
    emits('getName', fileList.value[0]?.name || '')
  }
}
</script>
